 ...
 * Must match the server-side implementation in server/utils/avatar.ts
 */
 ...
 * This matches the server-side implementation in server/utils/avatar.ts
 */
 ...
 * This matches the server-side implementation in server/utils/avatar.ts
 */
export function getUserAvatarUrl(user: { name: string; avatarUrl?: string; avatar_url?: string }): string {
  ...
  if (avatarUrl.includes('ui-avatars.com') && avatarUrl.includes('background=random')) {
    return generateAvatarUrl(user.name);
  }
