...
export const BoardView: React.FC<BoardViewProps> = ({ issues, users, onEdit, onDelete, onStatusChange, onCreateIssue, isPublicView }) => {
                ...
                {columnIssues.map(issue => {
                  ...
                  return (
                    <div
                      ...
                      onDragStart={(e) => !isPublicView && handleDragStart(e, issue.id)}
                          ...
                          <div className="flex -space-x-1.5 direction-rtl">
                            {assignees.slice(0, 3).map((u, i) => (
                              <img
                                ...
                                src={u.avatarUrl}
                                alt={u.name}
                                className="w-5 h-5 rounded-full border border-[#25262B] ring-1 ring-[#2E3036] z-[1]"
                                style={{ zIndex: 3 - i }}
                              />
                            ))}
                            {assignees.length > 3 && (
                              <div className="w-5 h-5 rounded-full bg-gray-700 border border-[#25262B] flex items-center justify-center text-[8px] text-white z-0">
                                +{assignees.length - 3}
                              </div>
                            )}
                          </div>
