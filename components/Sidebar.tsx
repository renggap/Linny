...
import { Inbox, Layers, Users, Search, Plus, Sparkles, ChevronDown, LogOut, Shield, Settings, StatusIcon } from './Icons';
...
interface SidebarProps {
  ...
  onSelectStatusFilter: (status: Status | null) => void;
...
export const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <aside className="w-[240px] bg-[#222328] border-r border-[#363840] flex flex-col h-full text-[#9CA3AF] text-[13px] font-medium select-none">
      ...
      <div className="flex-1 overflow-y-auto py-3 px-2 space-y-6">
        ...
        <div className="space-y-0.5">
          ...
          <div className="flex items-center px-2 py-1.5 rounded-md hover:bg-[#2E3036] cursor-pointer">
            <Inbox className="w-4 h-4 mr-2.5 opacity-70" />
            Inbox
            <span className="ml-auto bg-[#5E6AD2] text-white text-[10px] px-1.5 rounded-full">3</span>
          </div>
          
          <div 
            onClick={() => onSelectStatusFilter(null)} 
            className={`flex items-center px-2 py-1.5 rounded-md cursor-pointer transition-colors ${!selectedProjectId && !currentStatusFilter && !assigneeFilter ? 'bg-[#2E3036] text-white' : 'text-[#E5E7EB] hover:bg-[#2E3036]'}`}
          >
            <Layers className={`w-4 h-4 mr-2.5 ${!selectedProjectId && !currentStatusFilter && !assigneeFilter ? 'text-[#5E6AD2]' : ''}`} />
            All Issues
          </div>
          ...
          {statusViews.map(view => (
             <div 
                key={view.status}
                onClick={() => onSelectStatusFilter(view.status)}
                className={`flex items-center px-2 py-1.5 rounded-md cursor-pointer transition-colors ${currentStatusFilter === view.status ? 'bg-[#2E3036] text-white' : 'text-[#E5E7EB] hover:bg-[#2E3036]'}`}
             >
        ...
        {/* Projects */}
        <div>
          <div className="px-2 mb-1 text-[11px] font-semibold opacity-50 uppercase tracking-wider flex items-center justify-between group">
            Projects
            {canCreateContent && (
                <Plus onClick={onCreateProject} className="w-3 h-3 opacity-0 group-hover:opacity-50 cursor-pointer hover:text-white" />
            )}
          </div>
          <div className="space-y-0.5">
        ...
        {/* Team Members */}
        <div>
          <div className="px-2 mb-1 text-[11px] font-semibold opacity-50 uppercase tracking-wider">Team Members</div>
          <div className="space-y-0.5">
             ...
             >
               <div className="w-4 h-4 border border-dashed border-gray-600 rounded-full flex items-center justify-center mr-2.5">
                 {isAdmin ? <Settings className="w-2.5 h-2.5" /> : <Plus className="w-2.5 h-2.5" />}
               </div>
               {isAdmin ? 'Manage Team' : 'Invite people'}
             </div>
          </div>
        </div>
      ...
      <div className="p-3 border-t border-[#363840]">
        <div className="flex items-center px-2 py-1.5 rounded-md hover:bg-[#2E3036] cursor-pointer group relative">
